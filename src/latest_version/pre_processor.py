...
class Preprocessor:
    def __init__(self, file_key: str, environment: str, job_id: str, feature_method: tuple = ("RAW", 0),
        ...
        self.label_header = self.__generate_label_header()
        self.__label_encode()
    ...
    def __generate_label_header(self):
        for column in self.columns:
            if str(column).lower() == 'label':
                self.label_header = column
                break

        if not self.label_header:
    ...
    def save(self):
        ...
        output_path = f"{self.environment}/{self.job_id}/PROCESSED/{process_id}/processed.csv"
        csv_buf = StringIO()
